...
from .forms import ContactForm, BuyerForm, ProfileUpdateForm
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if buyer.is_valid():
            ...
            messages.error(request, Buyer.errors)
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if passupdate:
            ...
            messages.success(request, f'your password update is successful')
            return redirect('profile')
        else:
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_6f91884318ccb8d752494ee11d0a3c9b26dcc4a9' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://13.53.175.144/thankyou' #thank you page
        ...
        try:
            ...
            messages.error(request, 'network busy, tru again later')
        else:
